...
> __Отчет по НИР__ - создание _RTL_ модуля соортировки _radix-sort_ и дальнейшая проверка данного модуля на работоспособность при имплементации проекта под частоту тактирования _clk = 500 Мгц_.
...
Дано множество из M элементов - x_i, аждый элемент представлен в виде N-битного двоичного числа - x_ij, где i - порядковый номер числа, а j - порядковый номер бита разряда числа.
...
Для анализа изменения состояния бит от разряда к разряду во входных чмслах (именно по изменению битового значения определяется наименьшее/наибольшее значение из исходной выборки чисел) строится матрица H: для каждого элемента x_i вычисляется вектор h_i. Так,
...
На рис.3 приведена формула для рассчета z_j  - элемента формул представленной на рис.2, z_j - ключевой элемент модификации исходной матрицы, по сути выполняет функции bypass на случай, когда в определленом разряде у всех чисел находится бит соответствующий уровню "0". 
...
Изменения коснулись модуля вычисления матрицы H_modif (применена другая логическая операция для вычисления z_N-1 - эквиваленция __z_N-1 = and (~x_ij) or (and x_ij)__, изменена формула по вычислению h_ij: __h_ij = `h_i_j+1 ^ ~x_ij__, преобразована формула для нахождения N-1 строки матрицы H_modif - __`h_i_N+1 = z_N-1 or ~x_ij__), а также модуля selection block (при значении флага g_i = "1" - ksi_i не зануляется, а наоборот принимает максимально большое значение для числа данной разрядности).
...
> Ниже приведена таблица со значениями временных характеристик и используемых ресурсов разработанной системы при задданных конфигурациях проекта _radix-sort_. Имплементация проекта производилась при выбранном чипе xcku060-ffva1156-2-e (серия Kintex UltraScale).
...
### Заключение
При временном анализе алгоритма приведенной в статье на определенных числовых выборках с заданной разрядностью были получены неудовлетворительные временных характеристики для требуемой частоты тактирования.
